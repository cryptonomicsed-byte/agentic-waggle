	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"actions": []map[string]any{
			...
			{"name": "deposit", "method": "POST", "path": "/v1/signals",
				"params": map[string]string{"agent": "required", "resource": "required URI", "kind": "required (see signal_kinds)", "intensity": "0-10, default 1", "half_life_s": "decay half-life seconds, default 1800", "note": "free text for successors", "meta": "map[string]string"},
			...
			{"name": "gradient", "method": "GET", "path": "/v1/gradient",
				"params": map[string]string{"prefix": "URI prefix", "kind": "filter", "k": "top-k, default 20"},
				"doc":    "Ranked hotspots: where is the swarm's attention? Filter kind=help for a rescue map, kind=gold for a findings map."},
			...
			{"name": "watch_create", "method": "POST", "path": "/v1/watches",
				"params": map[string]string{"agent": "required", "name": "display name", "resource_prefix": "only ingest events whose resource starts with this prefix (e.g. 'tool://')"},
				"doc":    "Register an Ògún watch: a named channel that routes tool-outcome events to field signals. Returns {watch:{id,...}}."},
			{"name": "watches", "method": "GET", "path": "/v1/watches", "doc": "List registered watches."},
			{"name": "ingest", "method": "POST", "path": "/v1/ingest/{watch_id}",
				"params": map[string]string{"resource": "required (e.g. tool/scrape/https://...)", "outcome": "required: success | failure", "note": "free text"},
				"doc":    "Post a tool-execution outcome to a watch. Converts to a gold (success) or dead-end (failure) signal on the field."},
			{"name": "rings", "method": "GET", "path": "/v1/rings",
				"doc": "Sybil ring report: agent clusters sharing the same registration origin within the last 10 minutes, threshold 3."},
			...
			{"name": "status", "method": "GET", "path": "/v1/status", "doc": "Service health and live counts (includes watches, suspected_rings)."},
		},
		"auth": map[string]any{
			"mode":          "token — issued at POST /v1/agents, include as X-Waggle-Token: {agent_id}:{token}",
			"reads":         "open — no token required for GET endpoints",
			"writes":        "gated — X-Waggle-Token required; absent token is warned in open mode, rejected with 401 in --require-auth mode",
			"taboo":         "ed25519 capability embedded as body['capability'] = '<hex_payload>.<hex_sig>'; active when --taboo-auth-key is set",
			"mark_throttle": "30-burst / 0.5 marks per second per agent; 429 when exceeded",
		},
